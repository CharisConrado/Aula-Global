...
class StudentState:
    ...
    # Historial reciente de emociones (últimos 30 segundos, ~15 muestras a 2s)
...
class AdaptationEngine:
    ...
    def __init__(self):
        # Estado por estudiante: {student_id: StudentState}
        self._states: dict[int, StudentState] = {}

    def get_or_create_state(self, student_id: int, session_id: int) -> StudentState:
        """Obtiene o crea el estado de un estudiante."""
        key = student_id
        if key not in self._states or self._states[key].session_id != session_id:
            self._states[key] = StudentState(student_id=student_id, session_id=session_id)
        return self._states[key]

    def clear_state(self, student_id: int):
        """Limpia el estado de un estudiante (al cerrar sesión)."""
    ...
    def process(
        ...
        if emocion == Emocion.estresado:
            ...
            # Determinar nivel de crisis según intensidad y duración
            ...
            if nivel_crisis:
                ...
                if nivel_crisis == NivelCrisis.leve:
                    ...
                    acciones.append(AdaptationAction(
                        accion=AccionAdaptacion.adaptar_contenido,
                    ...
                    acciones.append(AdaptationAction(
                        ...
                        motivo="Crisis moderada detectada — se notifica al tutor",
                    ...
                    acciones.append(AdaptationAction(
                        ...
                        motivo="Crisis grave detectada — se requiere intervención profesional inmediata",
                        datos={"nivel_crisis": "grave", "duracion_estres": round(duracion_estres)},
                    ))
        else:
            state.estresado_desde = None

        # --- Regla adicional: Ansiedad + baja atención = posible crisis ---
    ...
    def _evaluar_crisis(
        ...
        if emociones_negativas >= 12:  # 80% del historial
            return NivelCrisis.grave

        # Crisis moderada: estrés > 30s o presión alta
        if duracion_estres > 30 or presion_tactil > 0.6:
            return NivelCrisis.moderada

        # Crisis leve: estrés detectado
